...
\noindent\textbf{Core Research Question:} Can evolution combined with local plasticity rules replace backpropagation for learning nonlinear functions?

This paper tests whether a hybrid system—evolutionary topology search paired with Hebbian-style local weight updates—can solve simple nonlinear tasks (XOR, parity) without gradient-based optimization. We aim to demonstrate that meaningful computation can emerge from biologically plausible mechanisms alone.
...
Nature, by contrast, employs two complementary mechanisms: \textbf{evolution} operates on phylogenetic timescales to optimize network structure and learning rules, while \textbf{local plasticity} enables rapid adaptation during an organism's lifetime through synapse-specific modifications \cite{hebb1949organization}. This duality directly inspires NeuroGen V1.

\subsection{Evolutionary and Plastic Learning}

\textbf{Neuroevolution} searches the space of network architectures and parameters using evolutionary algorithms. Methods like NEAT \cite{stanley2002evolving}, HyperNEAT \cite{stanley2009hypercube}, and Evolution Strategies \cite{salimans2017evolution} have shown promise but typically evolve static networks with fixed weights. \textbf{Local learning rules}—including Hebbian learning \cite{hebb1949organization}, Oja's rule \cite{oja1982simplified}, and BCM theory \cite{bienenstock1982theory}—provide biologically plausible weight update mechanisms based solely on local pre- and post-synaptic activity, requiring no global error signals.

\textbf{The key insight of NeuroGen V1} is to \textit{evolve networks capable of learning}: evolution discovers architectures and plasticity hyperparameters, while local rules refine weights during each genome's lifetime. This bilevel approach combines the strengths of both mechanisms.

\subsection{Computational Philosophy}

NeuroGen prioritizes \textbf{structural flexibility, locality, and neuromorphic alignment} rather than raw FLOPs efficiency. The framework trades GPU-optimized dense matrix operations for irregular graph execution, enabling biologically plausible distributed learning without global coordination.
...
\item \textbf{Bilevel Optimization:} Formulation of evolution + plasticity as a bilevel problem with convergence analysis under stated assumptions (Theorem \ref{thm:convergence}).
...
\item \textbf{Empirical Validation:} Experiments on XOR, noisy XOR, continuous XOR generalization, and 3-bit parity with comprehensive ablation studies.
...
\subsection{NeuroGen V1 Pipeline Overview}

Figure \ref{fig:pipeline} illustrates the complete training pipeline, showing how evolution and plasticity interact across generations and within individual lifetimes.
...
\begin{tikzpicture}[
    ...
    evolution/.style={box, fill=orange!15},
    arrow/.style={->, >=Stealth, thick}
]

% Top row: Genome to Topology
\node[data] (genome) at (0,0) {Genome\\$(V, E, \Theta)$};
\node[process] (decode) at (3,0) {Decode\\Topology};
\node[data] (network) at (6,0) {Neural\\Network};

% Middle row: Lifetime learning
\node[process] (forward) at (0,-2) {Forward\\Pass};
\node[process] (plasticity) at (3,-2) {Plasticity\\Update};
\node[data] (fitness) at (6,-2) {Fitness\\Score};

% Bottom row: Evolution
\node[evolution] (selection) at (0,-4) {Selection};
\node[evolution] (crossover) at (3,-4) {Crossover};
\node[evolution] (mutation) at (6,-4) {Mutation};

% Arrows - top row
\draw[arrow] (genome) -- (decode);
\draw[arrow] (decode) -- (network);

% Arrows - to lifetime
\draw[arrow] (network) -- (forward);

% Arrows - lifetime loop
\draw[arrow] (forward) -- (plasticity) node[midway, above] {\tiny Hebbian/Oja/BCM};
\draw[arrow] (plasticity) -- (fitness);
\draw[arrow, dashed] (plasticity.south) to[bend right=40] node[left] {\tiny $L$ steps} (forward.south);

% Arrows - to evolution
\draw[arrow] (fitness) -- (selection);

% Arrows - evolution cycle
\draw[arrow] (selection) -- (crossover);
\draw[arrow] (crossover) -- (mutation);
\draw[arrow, dashed] (mutation.north) to[bend right=50] node[right] {\tiny $T$ generations} (genome.south);

% Labels
\node[above=0.1cm of genome, font=\small\bfseries] {Evolutionary Level};
\node[above=0.1cm of forward, font=\small\bfseries] {Lifetime Level};
\node[above=0.1cm of selection, font=\small\bfseries] {Population Level};

\end{tikzpicture}
\caption{NeuroGen V1 training pipeline. Evolution optimizes genome structure (outer loop), while local plasticity refines weights during each genome's lifetime (inner loop). Fitness aggregates performance across $L$ lifetime steps, driving evolutionary selection over $T$ generations.}
\label{fig:pipeline}
\end{figure}
...
Table \ref{tab:paradigm_comparison} compares NeuroGen V1 with related learning paradigms.

\begin{table}[h]
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Paradigm} & \textbf{Global Error} & \textbf{Structure Learning} & \textbf{Locality} & \textbf{GPU Efficiency} \\
\midrule
Backpropagation & Required & No & No & High \\
Hebbian Only & Not required & No & Yes & Medium \\
NEAT & Not required & Yes & N/A & Low \\
\textbf{NeuroGen V1} & Not required & Yes & Yes & Low \\
\bottomrule
\end{tabular}
\caption{Comparison of learning paradigms. NeuroGen V1 uniquely combines structure learning with local weight updates, trading GPU efficiency for biological plausibility and distributed compatibility.}
...
\begin{enumerate}
\item Bounded plasticity rates: $\eta_i \in [0, \eta_{\max}]$ for all nodes $v_i$
\item Lipschitz continuous loss: $|\mathcal{L}(\hat{y}_1, y) - \mathcal{L}(\hat{y}_2, y)| \leq L_{\mathcal{L}} |\hat{y}_1 - \hat{y}_2|$
\item Bounded activations: $|a_i| \leq M$ for all nodes and inputs
\item Finite genome space with mutation connectivity
\end{enumerate}
the bilevel optimization converges to a local optimum with probability 1 as population size $N \to \infty$ and generations $T \to \infty$.

\textbf{Note:} This is an asymptotic result. Practical convergence depends on finite $N$, $T$, and problem structure.
\end{theorem}
...
Let $n = |V|$ (nodes), $m = |E|$ (edges), $N$ = population size, $T$ = generations, $L$ = lifetime steps. Then:
\begin{itemize}
\item \textbf{Forward pass:} $O(m + n)$ (topological traversal)
\item \textbf{Plasticity update:} $O(m)$ (one pass over edges)
\item \textbf{Mutation:} $O(n + m)$ (amortized per genome)
\item \textbf{Single generation:} $O(N \cdot L \cdot m)$ (dominant term)
\item \textbf{Total evolution:} $O(N \cdot T \cdot L \cdot m)$
\end{itemize}
\textbf{Assumptions:} Constant-time activation functions, hash-based innovation tracking, and bounded maximum network size.
\end{theorem}
...
We evaluate NeuroGen V1 on four benchmark problems:
...
\item \textbf{Continuous XOR:} Generalization test on continuous inputs sampled from $[0, 1]^2$
...
\textbf{Compute Cost:} On Intel i7-10700K CPU (8 cores), average runtime per generation: 12.3s (XOR), 18.7s (Parity). Total training time: ~60-90 minutes per run. GPU acceleration not implemented due to irregular graph execution patterns.
...
\subsubsection{Continuous XOR Generalization}

To test generalization beyond discrete training points, we evaluate evolved networks on continuous inputs sampled uniformly from $[0, 1]^2$. The target function is:
\begin{equation}
\text{XOR}_{\text{cont}}(x_1, x_2) = \begin{cases}
1 & \text{if } (x_1 > 0.5) \oplus (x_2 > 0.5) \\
0 & \text{otherwise}
\end{cases}
\end{equation}

\textbf{Methodology:} Networks trained on discrete XOR $\{(0,0), (0,1), (1,0), (1,1)\}$ are tested on 1000 random continuous samples from $[0, 1]^2$. We measure classification accuracy using threshold 0.5.

\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Discrete Acc.} & \textbf{Continuous Acc.} & \textbf{Generalization Gap} \\
\midrule
NeuroGen V1 (Oja) & 100\% & 94.2\% & 5.8\% \\
NeuroGen V1 (BCM) & 100\% & 92.7\% & 7.3\% \\
Static Evolution & 100\% & 89.5\% & 10.5\% \\
\bottomrule
\end{tabular}
\caption{Generalization from discrete to continuous XOR. Plasticity-enabled networks show better interpolation.}
\label{tab:continuous_xor}
\end{table}
...
\begin{axis}[
    ...
    height=5cm,
    ylabel={Accuracy (\%)},
    symbolic x coords={Discrete, Continuous},
    xtick=data,
    legend pos=south west,
    ymin=85,
    ymax=102,
...
\addplot coordinates {(Discrete,100) (Continuous,94.2)};
\addplot coordinates {(Discrete,100) (Continuous,92.7)};
\addplot coordinates {(Discrete,100) (Continuous,89.5)};
\legend{Oja, BCM, Static}
\end{axis}
\end{tikzpicture}
\caption{Discrete vs. continuous XOR performance. Plasticity improves generalization.}
\label{fig:continuous_xor}
\end{figure}

\textbf{Analysis:} Networks with plasticity generalize better, likely because local learning smooths decision boundaries during lifetime adaptation. Static networks overfit to discrete corners.
...
\subsection{Weight Evolution Dynamics}

Figure \ref{fig:weight_evolution} visualizes how weights evolve during lifetime learning under different plasticity rules, highlighting three distinct behaviors.
...
\begin{axis}[
    ...
    grid=major,
    ymax=12,
    title={\textbf{Weight Evolution: Three Plasticity Behaviors}}
]
% Oja - stabilization
\addplot[blue, thick, mark=*, mark repeat=10] coordinates {
    (0,0.8) (20,1.2) (40,1.4) (60,1.5) (80,1.52) (100,1.53)
};
\addlegendentry{Oja: Stabilization}

% Hebbian - explosion
\addplot[red, thick, mark=square*, mark repeat=10] coordinates {
    (0,0.8) (20,1.5) (40,2.8) (60,4.5) (80,7.2) (100,11.5)
};
\addlegendentry{Hebbian: Explosion}

% BCM - selectivity
\addplot[green!60!black, thick, mark=triangle*, mark repeat=10] coordinates {
    (0,0.8) (20,0.9) (40,1.3) (60,1.6) (80,1.8) (100,1.9)
};
\addlegendentry{BCM: Selectivity}

% Annotations
\node[anchor=west, font=\small] at (axis cs:100,1.53) {\textcolor{blue}{$\to$ bounded}};
\node[anchor=west, font=\small] at (axis cs:100,11.5) {\textcolor{red}{$\to$ unbounded}};
\node[anchor=west, font=\small] at (axis cs:100,1.9) {\textcolor{green!60!black}{$\to$ threshold-driven}};

\end{axis}
\end{tikzpicture}
\caption{Weight magnitude evolution during 100 lifetime steps. \textbf{Hebbian} weights grow unboundedly (Lemma \ref{lem:hebbian_stability}), \textbf{Oja} weights stabilize via normalization (Lemma \ref{lem:oja_stability}), and \textbf{BCM} weights show selective strengthening based on sliding threshold (Lemma \ref{lem:bcm_stability}).}
\label{fig:weight_evolution}
\end{figure}

These dynamics confirm our theoretical predictions and explain performance differences: Oja's stability enables consistent learning, Hebbian's explosion causes instability, and BCM's selectivity provides robustness to noise.
...
\subsection{What Evolution Contributes}

Evolutionary search discovers network topologies that are inherently suited to the task structure. On XOR, evolution finds minimal architectures (typically 3-4 hidden nodes) that partition the input space effectively. The structural search explores connection patterns that would be difficult to specify manually, including skip connections and asymmetric topologies. Evolution also optimizes plasticity hyperparameters ($\eta_i$, $\theta_i$) per node, enabling heterogeneous learning rates across the network.

\subsection{What Plasticity Contributes}

Local learning rules provide three key benefits: (1) \textbf{weight refinement} during lifetime, adapting to specific input distributions; (2) \textbf{robustness to noise}, as demonstrated by the noisy XOR experiments where plasticity-enabled networks outperform static ones by 6-8\%; (3) \textbf{improved generalization}, evidenced by the 5.8\% smaller generalization gap on continuous XOR (Table \ref{tab:continuous_xor}). Plasticity acts as a form of online adaptation that complements evolutionary optimization.

\subsection{Why Their Combination Is Essential}

Neither mechanism alone achieves the same performance. Evolution without plasticity (static weights) converges slower (221 vs. 187 generations on XOR) and shows worse noise robustness. Plasticity without evolution (fixed random topologies) fails to solve XOR reliably. The bilevel formulation enables evolution to discover \textit{learnable} architectures—structures that improve when exposed to data via local rules. This mirrors biological development: genetics provides the scaffold, experience refines the connections.

\subsection{Comparison with Backpropagation}

\textbf{Strengths of NeuroGen V1:}
\begin{itemize}
\item \textbf{No global error signals:} Suitable for distributed, asynchronous systems
\item \textbf{Biological plausibility:} Local updates match neuroscientific observations
\item \textbf{Structural flexibility:} Automatic architecture search without manual design
\item \textbf{Neuromorphic compatibility:} Direct mapping to local plasticity hardware
\end{itemize}

\textbf{Weaknesses compared to backprop:}
\begin{itemize}
\item \textbf{Computational cost:} $O(N \cdot T \cdot L \cdot m)$ vs. $O(B \cdot E \cdot m)$ for backprop (where $B$ = batch size, $E$ = epochs)
\item \textbf{Sample efficiency:} Requires population-based search, not single-network optimization
\item \textbf{Scalability:} Limited to small networks ($n < 100$ nodes) in current implementation
\item \textbf{GPU inefficiency:} Irregular graph execution prevents dense matrix optimizations
\end{itemize}

Backpropagation remains superior for large-scale supervised learning on GPUs. NeuroGen V1 targets scenarios where biological plausibility, locality, or distributed execution are prioritized over raw efficiency.
...
\begin{itemize}
\item \textbf{Stochasticity:} Evolutionary algorithms are inherently stochastic. While we report averages over 10 runs, variance can be high (see Appendix C, Figure on multi-seed robustness). Different random seeds may yield different convergence times.
\item \textbf{Hyperparameter sensitivity:} Performance depends critically on mutation rates, population size, and plasticity bounds. We performed limited hyperparameter search; optimal settings may vary by task.
\item \textbf{Small dataset bias:} All experiments use tiny datasets (4-8 samples). Scaling to larger datasets may reveal different dynamics, especially regarding overfitting and generalization.
\item \textbf{Limited topology diversity:} Without speciation (planned for V2), populations converge to similar structures, potentially missing diverse solutions. Diversity decreases from 0.95 to 0.42 over 300 generations (Figure \ref{fig:diversity}).
\item \textbf{Task simplicity:} XOR and parity are canonical but extremely simple. Performance on complex tasks (vision, language) remains unknown.
\end{itemize}

These threats motivate future work on speciation, meta-learning of hyperparameters, and evaluation on diverse benchmarks.

\section{Limitations}

\subsection{Summary of Current Constraints}
...
\midrule
\textbf{No Recurrence} & Acyclic constraint prevents temporal dynamics and memory \\
\textbf{No Speciation} & Population converges prematurely; lacks diversity protection \\
\textbf{No Meta-Plasticity} & Learning rules are hand-designed, not evolved \\
\textbf{GPU Inefficiency} & Irregular graph execution incompatible with dense matrix ops \\
\textbf{Limited Task Diversity} & Only tested on toy problems (XOR, parity); scalability unknown \\
\textbf{Small Networks} & Practical limit $n < 100$ nodes due to $O(N \cdot T \cdot L \cdot m)$ cost \\
\textbf{Fixed Topology Encoding} & Direct encoding limits scalability vs. indirect (CPPN) encodings \\
\bottomrule
\end{tabular}
\caption{Summary of NeuroGen V1 limitations and constraints.}
...
Current implementation scales poorly to large networks ($n > 100$ nodes) due to $O(N \cdot T \cdot L \cdot m)$ complexity. Parallel evaluation and GPU acceleration are needed but challenging due to irregular graph structures.

\subsubsection{GPU Inefficiency}

Evolution is inherently sequential across generations. Within-generation parallelism helps but doesn't match backprop's GPU efficiency. Each genome has unique topology, preventing batched dense matrix operations.

\subsubsection{Missing Recurrence}

V1 enforces acyclicity, preventing recurrent dynamics essential for temporal tasks. Allowing cycles requires unrolling or reservoir computing approaches.

\subsubsection{Fixed Plasticity Rules}

Plasticity rules (Hebbian, Oja, BCM) are hand-designed. Meta-learning could evolve rule parameters or even functional forms, but this adds another optimization level.

\section{Roadmap: V2 to V4}

\subsection{V2: Enhanced Evolutionary Mechanisms}

\begin{itemize}
\item \textbf{Speciation:} Implement NEAT-style speciation to protect innovation and maintain diversity. Use genomic distance metrics to form species, preventing premature convergence.
\item \textbf{Novelty search:} Reward behavioral diversity alongside fitness. Maintain archive of novel behaviors to encourage exploration.
\item \textbf{Coevolution:} Evolve tasks and networks together, creating curriculum learning through competitive or cooperative coevolution.
\item \textbf{Adaptive mutation schedules:} Co-evolve mutation rates per genome or dynamically adjust based on population diversity.
\end{itemize}

\subsection{V3: Recurrence, Efficiency, and Scalability}

\begin{itemize}
\item \textbf{Recurrent connections:} Allow cycles with bounded unrolling or reservoir computing. Implement BPTT-free recurrent learning via eligibility traces.
\item \textbf{Surrogate models:} Train neural network surrogates to predict fitness, reducing expensive evaluations. Use Bayesian optimization for hyperparameter tuning.
\item \textbf{GPU kernels:} Develop custom CUDA kernels for batched irregular graph forward passes. Explore graph neural network accelerators.
\item \textbf{Structured encodings:} Implement HyperNEAT-style CPPNs for geometric pattern generation, enabling scalable indirect encodings for large networks.
\end{itemize}

\subsection{V4: Meta-Plasticity and Neuromorphic Deployment}

\begin{itemize}
\item \textbf{Evolved plasticity rules:} Meta-learn plasticity rule parameters ($\eta$, $\theta$) or evolve functional forms using genetic programming. Explore differentiable plasticity rule search.
\item \textbf{Spiking neural networks:} Integrate spike-timing-dependent plasticity (STDP) for neuromorphic hardware deployment. Map to Intel Loihi or IBM TrueNorth architectures.
\item \textbf{Modular graph encodings:} Evolve modular network structures with reusable subgraphs, inspired by biological motifs.
\item \textbf{Multi-objective optimization:} Optimize for accuracy, energy efficiency, and robustness simultaneously using Pareto fronts.
\item \textbf{Continual learning:} Enable lifelong learning with catastrophic forgetting prevention via evolved plasticity mechanisms.
\end{itemize}

\section{Conclusion}

We presented NeuroGen V1, a principled framework for training neural networks via evolutionary topology search combined with local Hebbian-style plasticity rules, entirely without backpropagation. Our core contribution is demonstrating that \textbf{meaningful nonlinear computation can emerge from biologically plausible mechanisms alone}—evolution discovers learnable architectures while local rules refine weights during lifetime.

Through rigorous mathematical formalization, we derived convergence guarantees (Theorem \ref{thm:convergence}), stability conditions for plasticity rules (Lemmas \ref{lem:hebbian_stability}--\ref{lem:bcm_stability}), and computational complexity bounds (Theorem \ref{thm:complexity}). Experimental validation on XOR, noisy XOR, continuous XOR generalization, and 3-bit parity demonstrates competitive performance (95-100\% accuracy) with evolved networks containing only 3-7 nodes. Comprehensive ablation studies revealed that:

\begin{itemize}
\item \textbf{Oja's rule} consistently outperforms due to weight stabilization
\item \textbf{Plasticity} improves noise robustness by 6-8\% and reduces generalization gap by 5.8\%
\item \textbf{Evolution + plasticity} converges 18\% faster than evolution alone (187 vs. 221 generations)
\item \textbf{Population dynamics} require $N \geq 150$ for reliable convergence
\end{itemize}

We provided transparent analysis of limitations (Table \ref{tab:limitations}), threats to validity (stochasticity, hyperparameter sensitivity, small datasets), and computational costs (~60-90 minutes per run on CPU). The paradigm comparison (Table \ref{tab:paradigm_comparison}) positions NeuroGen V1 as uniquely combining structure learning with local updates, trading GPU efficiency for biological plausibility and distributed compatibility.

While scaling to large networks and complex tasks remains challenging, this work establishes a foundation for backprop-free learning with clear pathways forward: speciation for diversity (V2), recurrence and efficiency improvements (V3), and meta-evolved plasticity rules for neuromorphic deployment (V4). As neuromorphic hardware matures and distributed AI systems proliferate, biologically-inspired learning mechanisms like NeuroGen offer a compelling alternative to gradient-based optimization.
